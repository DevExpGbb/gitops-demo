...
resource "azuread_application" "github-workload-identity" {
    display_name = local.workload_name
...
resource "azuread_service_principal" "github-workload-identity" {
...
resource "azuread_app_role_assignment" "github-workload-identity" {
    ...
    principal_object_id = azuread_service_principal.github-workload-identity.object_id
...
resource "azuread_application_federated_identity_credential" "github-workload-identity" {
    ...
    display_name   = local.workload_name
    description    = "GH Workload Identity ${var.entity_type} ${local.environment_name}"
